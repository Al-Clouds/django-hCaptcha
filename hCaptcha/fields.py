...
class hCaptchaField(forms.CharField):
    default_error_messages = {
        'captcha_invalid': _('Incorrect captcha, please try again.')
    }
    ...
    def __init__(self, site_key=None, secret_key=None,
        ...
        self.widget = hCaptchaWidget(
            site_key=site_key, gtag_attrs=gtag_attrs, js_params=js_params)
        self.required = True
        super(hCaptchaField, self).__init__(*args, **kwargs)
    ...
    def clean(self, value):
        super(hCaptchaField, self).clean(value)
        hcaptcha_response_value = smart_unicode(value)
        if os.environ.get('HCAPTCHA_TESTING', None) == 'True' \
                and hcaptcha_response_value == 'PASSED':
            return value

        check_captcha = client.submit(
            hcaptcha_response_value, secret_key=self.secret_key,
